<template>
  <div class="forum-page">
    <div class="sg-page-title">
      ...
      <el-button v-if="userStore.token" type="primary" @click="showPublish = true">发布主题</el-button>
    </div>
    <el-dialog v-model="showPublish" title="发布主题" width="600px" @close="resetPublish">
      <el-form :model="publishForm">
        ...
        <el-form-item label="内容"><el-input v-model="publishForm.content" type="textarea" :rows="5" /></el-form-item>
      </el-form>
      <template #footer>
    ...
    <div class="ih-list-wrapper" v-loading="loading">
      <div v-for="item in list" :key="item.id" class="ih-list-item" @click="$router.push('/forum/'+item.id)">
      ...
      <div v-if="!loading && list.length === 0" class="ih-empty">暂无帖子，快来发布第一个主题吧</div>
...
// 论坛主题列表（路由：/forum）
// - 列表数据：GET /forum/topic/list（分页）
// - 发布主题：登录后弹窗发布，POST /forum/topic/save
// - 详情页：点击主题跳转 /forum/:id
...
const publishForm = reactive({ title: '', content: '', category: '', coverUrl: '' })

async function load() {
  loading.value = true
  try {
    const res = await request.get('/forum/topic/list', { params: { current: current.value, size: size.value } })
...
function resetPublish() {
  Object.assign(publishForm, { title: '', content: '', category: '', coverUrl: '' })
}
...
async function publish() {
  ...
  try {
    await request.post('/forum/topic/save', publishForm)
